...
export const ElementPreviewerProvider: React.FC<React.PropsWithChildren> = ({
  children,
}) => {
  const [breakPoint, setBreakPoint] = useState<TBreakPoints>(1024);
  const [mod, setMod] = useState<TMods>("preview");

  return (
    <ElementPreviewerContext.Provider
      value={{ breakPoint, setBreakPoint, mod, setMod }}
    >
      {children}
    </ElementPreviewerContext.Provider>
  );
